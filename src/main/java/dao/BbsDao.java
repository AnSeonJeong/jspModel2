...
public class BbsDao {
	...
	public boolean write(BbsDto dto) {
		...
		try {
			conn = DBConnection.getConnection();
			System.out.println("1/3 write success");
			
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, dto.getId());
			psmt.setString(2, dto.getTitle());
			psmt.setString(3, dto.getContent());
			System.out.println("2/3 write success");
			
			count = psmt.executeUpdate();
			System.out.println("3/3 write success");
	...
	public boolean answer(int seq, BbsDto dto) {
		...
		try {
			conn = DBConnection.getConnection();
			// commit을 비활성화	commit(=확정+적용) / rollback -> commit을 하면 rollback이 안됨
			...
			count1 = psmt.executeUpdate();	// update값이 0이 될 수도 있음
